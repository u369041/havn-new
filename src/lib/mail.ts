...
export async function sendListingStatusEmail(payload: ListingStatusEmailPayload) {
  try {
    ...
    const html = `
      ...
      ${safeHttpsUrl(payload.adminUrl) ? `<p><a href="${escapeAttr(safeHref(payload.adminUrl))}">Open in Admin</a></p>` : ""}
    ...
    logMailFailure("sendListingStatusEmail", err);
    return null;
  }
}
...
const HAVN_BASE_URL = "https://havn.ie";
...
function safeHttpsUrl(
  ...
  if (!raw || raw.length > 2_048) return fallback;
...
function safeHref(value?: string | null, fallback = HAVN_BASE_URL): string {
  return safeHttpsUrl(value, fallback) || fallback;
}

function safeCloudinaryImageUrl(value?: string | null): string | null {
  ...
  try {
    ...
    if (parsed.hostname.toLowerCase() !== CLOUDINARY_IMAGE_HOST) {
      return null;
    }
...
function logMailFailure(context: string, error: unknown) {
  const errorName =
...
function emailSafeCoverUrl(value?: string | null) {
  const safeUrl = safeCloudinaryImageUrl(value);
  if (!safeUrl) return null;
...
async function buildInlineCoverAttachment(value?: string | null) {
  ...
  try {
    ...
    if (!response.ok) {
      throw new Error("CoverImageRequestFailed");
    }
    ...
    if (
      ...
      declaredLength > MAX_COVER_IMAGE_BYTES
    ) {
    ...
    if (!bytes.length) {
      throw new Error("EmptyCoverImage");
    }
    ...
    console.warn("Could not embed Outlook-safe listing cover", {
      errorName:
                ...
                <tr>
                  <td style="padding:26px 30px 30px;background:#ffffff;">
                    ...
                    <table role="presentation" width="100%" cellspacing="0" cellpadding="0" style="width:100%;margin-top:24px;">
                      <tr>
                        <td align="center">
                          <a href="${escapeAttr(safeHref(args.publicUrl))}"
                             style="color:#000000;text-decoration:none;font-size:14px;font-weight:800;">
                            View My Listing
                          </a>
                        </td>
                ...
                <tr>
                  <td style="padding:0 30px 28px;background:#ffffff;">
                    ...
                    <table role="presentation" width="100%" cellspacing="0" cellpadding="0" style="width:100%;margin-top:22px;">
                      <tr>
                        <td align="center">
                          <a href="${escapeAttr(safeHref(args.editUrl))}" style="color:#000000;text-decoration:none;font-size:14px;font-weight:800;">Edit My Listing</a>
                        </td>
                      </tr>
                      <tr>
                        <td align="center" style="padding-top:14px;">
                          <a href="${escapeAttr(safeHref(args.myListingsUrl))}" style="color:#000000;text-decoration:none;font-size:13px;font-weight:800;">View My Listings</a>
                        </td>
                      </tr>
                    </table>
                  </td>
...
function renderHavnEmail(args: {
  ...
  const safeCoverImageUrl = safeCloudinaryImageUrl(args.coverImageUrl);
                    ...
                    ? `
                      <tr>
                        <td style="padding:0;line-height:0;">
                          <img src="${escapeAttr(safeCoverImageUrl || "")}" alt="Property" width="680"
...
export async function sendUserListingEmail(payload: UserListingEmailPayload) {
  try {
    ...
    logMailFailure("sendUserListingEmail", err);
    return null;
  }
}
...
export async function sendClosedListingEmail(payload: ClosedListingEmailPayload) {
  try {
    ...
    logMailFailure("sendClosedListingEmail", err);
    return null;
  }
}
...
export async function sendListingExpiresSoonEmail(payload: ListingExpiryEmailPayload) {
  try {
    ...
    logMailFailure("sendListingExpiresSoonEmail", err);
    return null;
  }
}
...
export async function sendListingExpiredEmail(payload: ListingExpiryEmailPayload) {
  try {
    ...
    logMailFailure("sendListingExpiredEmail", err);
    return null;
  }
}
...
export async function sendSavedSearchMatchEmail(args: {
  ...
  try {
    ...
    const propertyUrl = safeHref(args.propertyUrl);
    const price =
    ...
    const html = `
      ...
      <p><a href="${escapeAttr(propertyUrl)}">View property on HAVN</a></p>
    ...
    logMailFailure("sendSavedSearchMatchEmail", err);
    return null;
  }
}
...
export async function sendPropertyLeadEmail(payload: PropertyLeadEmailPayload) {
  try {
    ...
    if (!isValidEmail(payload.buyerEmail)) {
      throw new Error("InvalidReplyToEmail");
    }

    const propertyUrl = safeHttpsUrl(payload.propertyUrl);
    ...
    const html = `
      ...
      ${payload.slug ? `<p><strong>Slug:</strong> ${escapeHtml(String(payload.slug))}</p>` : ""}
      ${payload.listingId ? `<p><strong>ID:</strong> ${escapeHtml(String(payload.listingId))}</p>` : ""}
      ${propertyUrl ? `<p><a href="${escapeAttr(propertyUrl)}">View listing</a></p>` : ""}
    ...
    return await resend.emails.send({
      ...
      replyTo: payload.buyerEmail.trim(),
      subject,
      html,
    });
  } catch (err) {
    logMailFailure("sendPropertyLeadEmail", err);
    return null;
  }
}
...
export async function sendWelcomeEmail(args: { to: string; name?: string | null }) {
  try {
    ...
    logMailFailure("sendWelcomeEmail", err);
    return null;
  }
}
...
export async function sendPasswordResetEmail(args: {
  ...
  try {
    ...
    const resetUrl = safeHttpsUrl(args.resetUrl);
    if (!resetUrl) {
      throw new Error("InvalidPasswordResetUrl");
    }
    const displayName = String(args.name || "").trim();
    ...
    logMailFailure("sendPasswordResetEmail", err);
    return null;
  }
}
...
export async function sendEmailVerificationEmail(args: {
  ...
  try {
    ...
    if (!verifyUrl) {
      throw new Error("InvalidEmailVerificationUrl");
    }
    const displayName = String(args.name || "").trim();
    ...
    logMailFailure("sendEmailVerificationEmail", err);
    return null;
  }
}
...
export async function sendHavnWeeklyDigestEmail(args: {
  ...
  try {
    ...
    const matchesUrl = safeHref(args.matchesUrl);
    const manageAlertsUrl = safeHref(
      args.manageAlertsUrl,
      "https://havn.ie/my-listings.html"
    );
    const unsubscribeUrl = safeHttpsUrl(args.unsubscribeUrl);
    ...
    const propertiesHtml = (args.properties || []).slice(0, 4).map((p) => {
      const propertyUrl = safeHref(p.url);
      const imageUrl = safeCloudinaryImageUrl(p.imageUrl);
                ...
                <table role="presentation" width="170" cellspacing="0" cellpadding="0" style="width:170px;">
                  <tr>
                    <td width="170" height="120" style="width:170px;height:120px;overflow:hidden;background:#e5e7eb;line-height:0;font-size:0;">
                      ${
                        imageUrl
                          ? `<img src="${escapeAttr(imageUrl)}" alt="${escapeAttr(p.title)}" width="170" height="120" style="display:block;width:170px;height:120px;border:0;outline:none;text-decoration:none;" />`
            ...
            <tr>
              <td style="padding:12px;">
                ...
                <div style="margin-top:12px;">
                  <a href="${escapeAttr(propertyUrl)}" style="display:inline-block;background:#4f46e5;color:#ffffff;text-decoration:none;font-size:12px;font-weight:800;padding:9px 14px;border-radius:8px;">
                    View property →
                  </a>
                </div>
              </td>
              ...
              <table role="presentation" width="760" cellspacing="0" cellpadding="0" style="width:760px;max-width:100%;background:#ffffff;border-radius:18px;overflow:hidden;border:1px solid #e5e7eb;">
                    ...
                    <table role="presentation" width="100%">
                      <tr>
                        ...
                        <td align="right" style="font-size:13px;">
                          <a href="${escapeAttr(matchesUrl)}" style="color:#e0e7ff;text-decoration:underline;">View in browser</a>
                        </td>
                      </tr>
                    </table>
                  </td>
                ...
                <tr>
                  <td style="padding:42px 34px 30px;background:linear-gradient(90deg,#ffffff 0%,#ffffff 50%,#eff6ff 100%);">
                    ...
                    <a href="${escapeAttr(matchesUrl)}" style="display:inline-block;background:#4f46e5;color:#ffffff;text-decoration:none;font-weight:900;padding:15px 22px;border-radius:9px;">
                      View all matches →
                    </a>
                  </td>
                </tr>
                        ...
                        <td style="padding:12px 24px 8px;">
                          <table role="presentation" width="100%">
                            <tr>
                              ...
                              <td align="right">
                                <a href="${escapeAttr(matchesUrl)}" style="color:#4f46e5;font-weight:900;text-decoration:none;">View all (${newMatchesCount}) →</a>
                              </td>
                            </tr>
                          </table>
                        </td>
                ...
                <tr>
                  <td style="padding:0 24px 24px;">
                    <table role="presentation" width="100%" cellspacing="0" cellpadding="0">
                      <tr>
                        ${insightCard("PRICE DROPS", `${priceDropsCount} properties with price reductions`, "Save on your next move", "View price drops →", matchesUrl)}
                        ${insightCard("TRENDING THIS WEEK", `${trendingAreasCount || 3} areas are heating up`, "See where buyer demand is moving", "Explore areas →", matchesUrl)}
                        ${insightCard("FEATURED HOMES", `${featuredCount} homes hand-picked for you`, "Don’t miss these", "See featured homes →", matchesUrl)}
                      </tr>
                    </table>
                  </td>
                </tr>
                    ...
                    <table role="presentation" width="100%" style="background:#f5f3ff;border:1px solid #ede9fe;border-radius:16px;">
                      <tr>
                        ...
                        <td align="right" style="padding:20px;">
                          <a href="${escapeAttr(manageAlertsUrl)}" style="display:inline-block;border:1px solid #4f46e5;color:#4f46e5;text-decoration:none;font-weight:900;padding:13px 18px;border-radius:9px;">Manage alerts →</a>
                        </td>
                      </tr>
                    </table>
                  </td>
...
${
  unsubscribeUrl
    ? `<span style="float:right;"><a href="${escapeAttr(unsubscribeUrl)}" style="color:#cbd5e1;">Unsubscribe</a></span>`
    : `<span style="float:right;"><a href="${escapeAttr(manageAlertsUrl)}" style="color:#cbd5e1;">Manage alerts</a></span>`
}
                    </div>
                  </td>
                </tr>
    ...
    logMailFailure("sendHavnWeeklyDigestEmail", err);
    return null;
  }
}
  ...
  return `
    <td style="width:33.333%;padding:8px;vertical-align:top;">
      <div style="background:#f8fafc;border:1px solid #e5e7eb;border-radius:16px;padding:18px;min-height:138px;">
        ...
        <a href="${escapeAttr(href)}" style="color:#4f46e5;text-decoration:none;font-weight:900;font-size:13px;">${escapeHtml(cta)}</a>
      </div>
    </td>
  `;
}
