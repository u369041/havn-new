...
export function buildDemoProperties(count = 25): DemoProp[] {
  ...
  for (let i = 0; i < count; i++) {
    ...
    const price = 900 + Math.floor(Math.random() * 3500); // 900–4400
    ...
    const photos = Array.from({ length: numPhotos }, (_, idx) =>
      // stable placeholder image (seeded) — avoids nulls
    ...
    out.push({ slug, title, price, photos, features });
  }
  return out;
}
